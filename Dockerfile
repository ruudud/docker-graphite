...
RUN apt-get update && apt-get install -y python-cairo collectd libgcrypt11 python-virtualenv supervisor sudo build-essential python-dev openssh-server openssh-client python-pip && apt-get clean && rm -rf /var/cache/apt/archives/* /var/lib/apt/lists/*
